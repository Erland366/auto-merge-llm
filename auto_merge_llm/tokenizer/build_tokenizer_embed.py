...
import traceback
from auto_merge_llm.utils import logger
        ...
        for index, (model, tokenizer, model_vocab_size) in enumerate(
            ...
            for added_pad_token in added_pad_tokens_set:
                ...
                if num_new_tokens > 0:
                    ...
                    # Only handle output embeddings if they exist
                    if output_embeddings is not None:
                        output_embeddings = output_embeddings.weight.data
                        output_embeddings_avg = output_embeddings[:-num_new_tokens].mean(dim=0, keepdim=True)
                        output_embeddings[-num_new_tokens:] = output_embeddings_avg
            
            # Get output embeddings shape safely (some models don't have output embeddings)
            output_shape = "N/A"
            if model.get_output_embeddings() is not None:
                output_shape = model.get_output_embeddings().weight.shape
            
            logger.info(f"Aligned index {index} model: input token embedding shape {model.get_input_embeddings().weight.shape}, "
                        f"output token embedding shape {output_shape}, "
                        f"tokenizer added tokens {json.loads(tokenizer._tokenizer.to_str())['added_tokens']}.")
            # Handle different model architectures
            if hasattr(model, 'model') and hasattr(model.model, 'embed_tokens'):
                # For decoder models like LLaMA
                embed_shape = model.model.embed_tokens.weight.shape
            elif hasattr(model, 'roberta') and hasattr(model.roberta, 'embeddings'):
                # For encoder models like RoBERTa/XLM-RoBERTa
                embed_shape = model.roberta.embeddings.word_embeddings.weight.shape
            elif hasattr(model, 'embeddings'):
                # For some other encoder models
                embed_shape = model.embeddings.word_embeddings.weight.shape
            else:
                # Fallback
